  ...
  <div class="wrap">
      ...
      <div class="key-settings" id="key-settings">
        <div class="key-settings-row">
          ...
          <label class="field">Network
            <select id="network"><option value="mainnet" disabled>Bitcoin mainnet</option><option value="testnet" selected>Testnet (practice)</option></select>
          </label>
        </div>
    ...
    <section class="card no-print" id="msig-card" role="tabpanel" hidden>
      ...
      <div class="row msig-settings-row" style="align-items:end">
        ...
        <label class="field" style="margin:0;flex:1">People / devices (n)
          <select id="msig-n"></select>
        </label>
        <label class="field" style="margin:0;flex:1">Network
          <select id="msig-network"><option value="mainnet" disabled>Bitcoin mainnet</option><option value="testnet" selected>Testnet (practice)</option></select>
        </label>
        <label class="field" style="margin:0;flex:1">Addresses
      ...
      <div class="row current-item-actions">
        <button class="btn primary" id="msig-go" type="button">Build Multisig</button>
      ...
      <div class="psbt-grid">
        ...
        <div>
          ...
          <label class="field">Address network
            <select id="psbt-network"><option value="mainnet" disabled>Bitcoin mainnet</option><option value="testnet" selected>Testnet (practice)</option></select>
          </label>
        </div>
      </div>
...
function hodlAccountAdvancedExports(account,includePrivate=false){
  ...
  return`<details class="wallet-advanced"><summary>Advanced ${includePrivate?"private":"watch-only"} export</summary>${privateExport||publicExport}</details>`
}
...
function hodlUpdateDerivationPathPreview(){
  ...
  let network=hodlForceTestnet(document.getElementById("network")),count=Math.min(Math.max(Number(document.getElementById("count")?.value)||5,1),50),last=count-1;
...
function hodlInitDerivationControls(){
  ...
  panel.addEventListener("change",event=>{
    ...
    if(target.id==="network"||target.id==="count"){
      ...
      hodlInvalidateLiveKeyResult();let error=document.getElementById("error");if(error)error.textContent="";hodlUpdateDerivationPathPreview();if(target.id==="network"){let seed=document.getElementById("seed");if(seed)seed.dispatchEvent(new Event("input"))}
    }
  });
...
function hodlRenderKeyForm(){
  ...
  if(Ne==="seed"){
    ...
    let input=document.getElementById("seed"),update=()=>{
      ...
      if(hodlLooksExtendedKey(value)){let status=hodlSinglesigImportStatus(value,hodlForceTestnet(document.getElementById("network")));picker.innerHTML="";meta.textContent=status.message;meta.className="muted "+(status.ok?"ok":"err");return}
...
function hodlBindKeyFields(){
  ...
  let key=document.getElementById("key");if(key){let apply=()=>{let kind=document.querySelector("input[name=kk]:checked")?.value||"wif-or-hex";if(kind!=="brain")key.value=hodlFilterKey(key.value,kind);let value=key.value.trim();if(!value){hodlHint(key,!0,"");return}if(kind==="brain"){hodlHint(key,!1,"Brain wallets are unsafe. Recovery only.");return}if(kind==="minikey"){try{Ns(value);hodlHint(key,!0,"Mini key checksum looks valid")}catch(error){hodlHint(key,!1,error.message||"Not a valid mini key")}return}if(/^[0-9a-fA-F]{64}$/.test(value)){hodlHint(key,!0,"64-character hex private key");return}try{let decoded=Ls(value);if(decoded.network!=="testnet")throw new Error("Mainnet WIF keys are temporarily disabled. Use a Testnet WIF key.");hodlHint(key,!0,"Testnet WIF private key checksum looks valid")}catch(error){hodlHint(key,!1,error.message||"Not a Testnet WIF key or 64-character hex")}};key.oninput=apply;document.querySelectorAll("input[name=kk]").forEach(radio=>radio.addEventListener("change",apply))}
}
...
function hodlPrivateKeyInputIsValid(){
  ...
  try{
    ...
    let decoded=Ls(value);if(decoded.network!=="testnet")return!1;hf(decoded.priv);return!0
  }catch{return!1}
}
function hodlCanDeriveCurrentKey(){
  try{
    ...
    if(Ne==="seed"){
      ...
      if(hodlLooksExtendedKey(value))return hodlUsableSinglesigImport(value,hodlForceTestnet(document.getElementById("network")));
...
function hodlCalculateKey(){
  W("#error").textContent="";
  try{
    let network=hodlForceTestnet(document.getElementById("network")),count=Number(document.getElementById("count").value),passphrase=document.getElementById("pass").value,scriptType=hodlSelectedScriptType(),account=Ne==="key"?0:hodlReadAccount();
    if(Ne==="dice"){
      if(ge==="dplus"){
    ...
    if(re?.network!=="testnet")throw new Error("Mainnet key material is temporarily disabled. Use Testnet-formatted key material.");
...
function hodlCheckXpub(ta){let value=ta.value.trim();if(!value){hodlHint(ta,!0,"");return}try{let parsed=uf(value),network=hodlForceTestnet(document.getElementById("msig-network")),kind=hodlScriptKind();if(parsed.isPrivate)throw new Error("Paste an extended public key, never an extended private key.");if(parsed.network!==network)throw new Error(`${parsed.prefix} is for ${parsed.network}; the multisig is set to ${network}.`);if(!hodlMultisigPrefixCompatible(parsed,kind))throw new Error(parsed.scope==="singlesig"?"Use a generic xpub/tpub here, or a proper uppercase multisig SLIP-132 export.":`${parsed.prefix} does not match the selected multisig script type.`);let accountError=hodlMultisigAccountKeyError(parsed,kind);if(accountError)throw new Error(accountError);if(hodlDuplicateMultisigKey(ta,parsed))throw new Error("This duplicates another co-signer. Every co-signer must use a distinct extended public key.");hodlHint(ta,!0,`${parsed.prefix} checksum, network, and account path look valid`)}catch(error){hodlHint(ta,!1,error.message||"Not a valid multisig extended public key")}}
function hodlResetMsigForm(){hodlSyncSelect(document.getElementById("msig-n"),"3");hodlFillM(2);document.querySelectorAll("input[name=msig-script]").forEach(input=>{input.checked=input.value==="p2wsh"});hodlFillKeys([]);hodlSyncSelect(document.getElementById("msig-network"),"testnet");hodlSyncSelect(document.getElementById("msig-count"),"5");document.getElementById("msig-error").textContent=""}
...
function hodlBuildMsig(){
  ...
  try{
    let network=hodlForceTestnet(document.getElementById("msig-network")),count=Number(document.getElementById("msig-count").value),n=Number(document.getElementById("msig-n").value),m=Number(document.getElementById("msig-m").value);
...
function hodlLoadPsbtKey(text,passphrase){
  ...
  if(/^[5KL9c][1-9A-HJ-NP-Za-km-z]{50,51}$/.test(value)){let decoded=Ls(value);if(decoded.network!=="testnet")throw new Error("Mainnet WIF keys are temporarily disabled. Use a Testnet WIF key.");hodlPsbtPriv=decoded.priv;hf(hodlPsbtPriv);hodlPsbtNote="Session key: WIF. Kept in page memory only."}
...
function hodlInitPsbt(){
  let go=document.getElementById("psbt-go");if(!go)return;hodlForceTestnet(document.getElementById("psbt-network"));go.onclick=hodlRunPsbt;
...
function hodlRunPsbt(){
  ...
  try{hodlForceTestnet(document.getElementById("psbt-network"));if(manual.trim()){hodlLoadPsbtKey(manual,document.getElementById("psbt-pass").value);document.getElementById("psbt-key").value="";document.getElementById("psbt-pass").value=""}document.getElementById("psbt-session").textContent=hodlPsbtNote;let psbt=hodlParsePsbt(hodlPsbtBytes(document.getElementById("psbt-text").value));output.innerHTML=hodlRenderPsbt(psbt)}catch(exception){error.textContent=exception instanceof Error?exception.message:String(exception)}
}
function hodlRenderPsbt(psbt){
  let network=hodlForceTestnet(document.getElementById("psbt-network")),tx=psbt.tx,inputSum=0n,knownInputs=0,html=[],rValues=[],rows=[],tapSignatureCount=0;
...
function hodlKeyStateNeedsClear(state){
  ...
  return String(state.mode??"dice")!=="dice"||String(state.diceMethod??"coldcard")!=="coldcard"||String(state.entropyFormat??"hex")!=="hex"||Boolean(state.seedAutocomplete)||Boolean(state.dplusNumberedD16)||Number(state.targetWords??24)!==24||
    ...
    String(state.accountId??"bip84")!=="bip84"||String(fields.script??"bip84")!=="bip84"||String(fields.network??"testnet")!=="testnet"||String(fields.account??"0")!=="0"||String(fields.count??"5")!=="5"||
...
function hodlCaptureKey(){
  ...
  ["pass","account","count","hex","bin","seed","key"].forEach(id=>{let el=document.getElementById(id);if(el)state.fields[id]=el.value});state.fields.network="testnet";
...
function hodlForceTestnet(select){
  if(select&&select.value!=="testnet")hodlSyncSelect(select,"testnet");
  return"testnet"
}
function hodlRestoreKey(){
  ...
  if(!state){
    ...
    hodlSyncSelect(document.getElementById("script-type"),"bip84");hodlSyncSelect(document.getElementById("network"),"testnet");let account=document.getElementById("account");if(account)account.value="0";hodlSyncSelect(document.getElementById("count"),"5");
  ...
  state.fields.network="testnet";hodlSyncSelect(document.getElementById("network"),"testnet");
...
function hodlNewMsigState(name,msigId,msigNumber){let id=msigId??hodlNextMsigId++,number=msigNumber??hodlNextMsigNumber++;return{id,number,name:name||hodlDefaultMsigName(number),result:null,error:"",fields:{m:"2",n:"3",script:"p2wsh",xpubs:["","",""],network:"testnet",count:"5"}}}
function hodlMsigStateNeedsClear(state){
  ...
  return Boolean(state.result)||String(state.error??"").length>0||xpubs.some(value=>String(value??"").length>0)||
    String(fields.m??"2")!=="2"||String(fields.n??"3")!=="3"||String(fields.script??"p2wsh")!=="p2wsh"||String(fields.network??"testnet")!=="testnet"||String(fields.count??"5")!=="5"
}
...
function hodlCaptureMsig(){
  ...
  state.fields.n=document.getElementById("msig-n").value||"3";state.fields.m=document.getElementById("msig-m").value||"2";state.fields.script=hodlScriptKind();hodlMergeMsigXpubs(state);state.fields.network="testnet";state.fields.count=document.getElementById("msig-count").value||"5";
...
function hodlRestoreMsig(){
  ...
  hodlFillKeys(state.fields.xpubs||[]);state.fields.network="testnet";hodlSyncSelect(document.getElementById("msig-network"),"testnet");hodlSyncSelect(document.getElementById("msig-count"),state.fields.count||"5");
