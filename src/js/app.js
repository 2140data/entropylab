...
var hodlEntropyFormats=Object.freeze({
  ...
  base32:Object.freeze({id:"base32",base:32,bitsPerDigit:5,alphabet:"0123456789ABCDEFGHJKMNPQRSTVWXYZ",label:"Crockford Base32",shortLabel:"Base32",unit:"characters",method:"base32",binaryRemainder:!0}),
  base64:Object.freeze({id:"base64",base:64,bitsPerDigit:6,alphabet:"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/",label:"Base64 (RFC 4648 alphabet)",shortLabel:"Base64",unit:"characters",method:"base64",binaryRemainder:!0})
});
...
function hodlNormalizeEntropyFormat(format){return Object.hasOwn(hodlEntropyFormats,String(format??""))?String(format):"bin"}
...
function hodlDPlusTokens(value){
  let text=String(value??""),entries=[],index=0;
  while(index<text.length){
    let character=String.fromCodePoint(text.codePointAt(index));
    if(/[\s,;|]/.test(character)){index+=character.length;continue}
    entries.push({face:character.toUpperCase(),start:index,end:index+character.length});
...
function hodlDPlusRolls(value,targetWords=Pt,numberedD16=hodlDPlusNumberedD16){
  ...
  let finalEntry=config.words===24||config.words===12?entries[rolledCharacterTarget]||null:null,finalRoll="";
  ...
  if(finalCoinEntry){
    if(/^[1-8]$/.test(finalCoinEntry.face)){finalCoin=finalCoinEntry.face;acceptedCharacters.push(finalCoinEntry.face);bits+=1}
    else{invalidRanges.push([finalCoinEntry.start,finalCoinEntry.end]);invalidRequiredCount+=1;if(!firstInvalid)firstInvalid={groupIndex:rolledTarget,position:1,face:finalCoinEntry.face,start:finalCoinEntry.start,end:finalCoinEntry.end,final:!0}}
  }
  if(finalEntry){
  ...
  if(finalD16&&finalCoin&&finalWord)notes.push(`Final D16 result ${finalD16} and coin flip ${finalCoin} (${Number(finalCoin)>=5?"Tails":"Heads"}) selected checksum option ${hodlDPlusD16Value(finalD16,numberedD16)*2+(Number(finalCoin)>=5?1:0)+1} of ${config.candidates}: ${finalWord}.`);
  ...
  return{words,wordSlots,groups,group,entries,finalEntry:finalEntry?.face||"",finalRoll,finalD16,finalCoin,finalWord,candidates,waiting,currentPosition,activeGroupIndex,completedGroups,validWordCount,allRolledComplete,allRolledValid,bits,notes,warnings,invalidRanges,invalidCount:invalidRanges.length,invalidRequiredCount,rolledInvalidCount,needsCorrection:invalidRequiredCount>0,firstInvalid,rejectedD8,rejectedD16,extraAfter,acceptedCharacters,targetWords:config.words,neededPartial:rolledTarget,numberedD16,complete:allRolledValid&&Boolean(finalWord)}
}
...
function hodlDiceControlValue(button){
  if(!button.dataset.coin)return button.dataset.d||"";
  let low=Number(button.dataset.coinLow),high=Number(button.dataset.coinHigh);
  if(!Number.isInteger(low)||!Number.isInteger(high))[low,high]=button.dataset.coin==="heads"?[1,3]:[4,6];
  return String(hodlRandomDiceFace(low,high))
}
...
function hodlDPlusSeparator(index,seed,numberedD16){
  if(index===0)return"";
  let rolled=seed.partialWords*3,wordBoundary=index<rolled?index%3===0:index===rolled;
  return wordBoundary?" ":""
}
function hodlSanitizeDPlusInput(input,targetWords=Pt,numberedD16=hodlDPlusNumberedD16){
  ...
  tokens.forEach((token,index)=>{
    ...
    for(let character of token){clean+=character;significantEnds.push(clean.length)}
  });
  let countSignificant=value=>{let count=0;for(let character of String(value))if(allowed.test(character))count+=1;return count},total=significantEnds.length-1;
...
function hodlRenderInputHighlight(input,ranges=[]){
  let highlight=input.closest(".dice-input-shell")?.querySelector(".dice-input-highlight");if(!highlight)return;let fragment=document.createDocumentFragment(),cursor=0,normalized=ranges.map(range=>[Math.max(0,Number(range[0])||0),Math.min(input.value.length,Number(range[1])||0),range[2]||"dice-roll-invalid"]).filter(([start,end])=>end>start).sort((a,b)=>a[0]-b[0]);
  normalized.forEach(([rangeStart,rangeEnd,className])=>{let start=Math.max(cursor,rangeStart),end=Math.max(start,rangeEnd);if(start>cursor)fragment.appendChild(document.createTextNode(input.value.slice(cursor,start)));if(end>start){let span=document.createElement("span");span.className=className;span.textContent=input.value.slice(start,end);fragment.appendChild(span);cursor=end}});
...
function hodlNormalizeEntropyCharacter(character,format){let id=hodlNormalizeEntropyFormat(format),normalized=String(character??"");if(id==="base64")return normalized;normalized=normalized.toUpperCase();if(id==="base32"){if(normalized==="O")return"0";if(normalized==="I"||normalized==="L")return"1"}return normalized}
function hodlFilterNumberBase(value,format){let meta=hodlEntropyFormatConfig(format),filtered="";for(let character of String(value??"")){if(/\s/.test(character)){filtered+=character;continue}let normalized=hodlNormalizeEntropyCharacter(character,meta.id);if(meta.alphabet.includes(normalized))filtered+=normalized}return filtered}
function hodlEntropyDigitEntries(value,format){let meta=hodlEntropyFormatConfig(format),entries=[],invalidEntries=[];for(let index=0;index<String(value??"").length;){let character=String.fromCodePoint(String(value).codePointAt(index)),end=index+character.length;if(!/\s/.test(character)){let normalized=hodlNormalizeEntropyCharacter(character,meta.id),digit=meta.alphabet.indexOf(normalized),entry={character,normalized,digit,start:index,end};if(digit<0)invalidEntries.push(entry);else entries.push(entry)}index=end}return{entries,invalidEntries}}
function hodlEntropyDigits(value,format){return hodlEntropyDigitEntries(value,format).entries.map(entry=>entry.normalized).join("")}
function hodlNumberBaseBits(value,format,targetWords=Pt){let meta=hodlEntropyFormatConfig(format,targetWords),digits=hodlEntropyDigits(value,meta.id).slice(0,meta.digits);return Array.from(digits,(character,index)=>{if(meta.binaryRemainder&&index>=meta.fullDigits)return character;let width=meta.remainderBits&&index===meta.digits-1?meta.remainderBits:meta.bitsPerDigit;return meta.alphabet.indexOf(character).toString(2).padStart(width,"0")}).join("").slice(0,meta.seed.bits)}
function hodlNumberBaseValueFromBytes(bytes,format,targetWords=Pt){let meta=hodlEntropyFormatConfig(format,targetWords),bits=Array.from(bytes,byte=>byte.toString(2).padStart(8,"0")).join(""),value="";for(let index=0;index<meta.fullDigits;index++){let start=index*meta.bitsPerDigit;value+=meta.alphabet[Number.parseInt(bits.slice(start,start+meta.bitsPerDigit),2)]}if(meta.remainderBits){let finalBits=bits.slice(meta.fullDigits*meta.bitsPerDigit);value+=meta.binaryRemainder?finalBits:meta.alphabet[Number.parseInt(finalBits,2)]}return meta.id==="bin"?hodlGroupedBinary(value):value}
...
function hodlRenderEntropyInputState(input,format,targetWords=Pt){let analysis=hodlAnalyzeEntropyInput(input.value,format,targetWords),invalid=analysis.invalidRanges.length>0;input.classList.toggle("bad",invalid);input.setAttribute("aria-invalid",String(invalid));hodlRenderInputHighlight(input,analysis.invalidRanges);return analysis}
function hodlUpdateDiceButtons(input,analysis){
  let form=input.closest("#form"),pad=form?.querySelector(".dice-input-pad");
  // The final coin flip only exists once every D16 roll is in, so its pad stays
  // out of the way until that turn rather than sitting there disabled.
  let coinPad=form?.querySelector(".dplus-coin-pad");
  if(coinPad)coinPad.hidden=!(ge==="dplus"&&analysis.dplus?.waiting==="checksum-coin");
  if(!pad)return;pad.querySelectorAll("button[data-d]").forEach(button=>{
    let disabled=!1,reason="";
    if(ge==="dplus"){
      let turn=analysis.dplus?.waiting||"d8",isD8=turn==="d8"||turn==="checksum-d8",coinTurn=turn==="checksum-coin",correcting=turn==="correction",value=String(button.dataset.d||"").toUpperCase(),isCoin=/^[HT]$/.test(value);
      disabled=turn==="complete"||turn==="last-word"||correcting||(coinTurn?!/^[1-8]$/.test(value):isCoin||(isD8&&!/^[1-8]$/.test(value))||(!isD8&&hodlDPlusD16Value(value)===null));
      ...
      else if(coinTurn&&disabled)reason="The final flip is a D8 roll: 1–4 is Heads, 5–8 is Tails.";
      else if(coinTurn)reason="Final flip: 1–4 is Heads, 5–8 is Tails.";
...
function hodlNumberBaseEntropy(value,format,targetWords=Pt){
  ...
  if(!analysis.count)return{ok:!1,error:`Enter exactly ${meta.digits} ${meta.unit} for a ${meta.seed.words}-word seed.`,notes,warnings};
  if(analysis.invalidCharacterCount)return{ok:!1,error:`${meta.shortLabel} entropy contains ${analysis.invalidCharacterCount} invalid character${analysis.invalidCharacterCount===1?"":"s"}.`,notes,warnings};
  if(analysis.finalInvalid)return{ok:!1,error:meta.binaryRemainder?`The final ${meta.remainderBits} ${meta.shortLabel} entropy bit${meta.remainderBits===1?"":"s"} must each be 0 or 1.`:`The final ${meta.shortLabel} character contributes only ${meta.remainderBits} bit${meta.remainderBits===1?"":"s"} and must be one of ${[...meta.finalCharacters].join(", ")}.`,notes,warnings};
  if(analysis.count!==meta.digits)return{ok:!1,error:`The selected ${meta.seed.words}-word seed needs exactly ${meta.digits} ${meta.unit} (${meta.seed.bits} bits). You entered ${analysis.count}.`,notes,warnings};
  let bits=hodlNumberBaseBits(value,meta.id,meta.seed.words),bytes=new Uint8Array(meta.seed.bytes);for(let index=0;index<bytes.length;index++)bytes[index]=Number.parseInt(bits.slice(index*8,index*8+8),2);
  notes.push(`${meta.digits} ${meta.unit} = ${meta.seed.bits} bits of ${meta.shortLabel} entropy.`);if(meta.remainderBits)notes.push(meta.binaryRemainder?`${meta.fullDigits} complete ${meta.shortLabel} characters are followed by ${meta.remainderBits} individual coin-flip entropy bit${meta.remainderBits===1?"":"s"}.`:`The final character is mixed-radix: it contributes the remaining ${meta.remainderBits} entropy bit${meta.remainderBits===1?"":"s"} and must be one of ${[...meta.finalCharacters].join(", ")}.`);notes.push(`BIP39 entropy length: ${meta.seed.bits} bits → ${meta.seed.words}-word seed.`);
  return{ok:!0,bytes,hex:M.encode(bytes),bits:meta.seed.bits,sourceBits:meta.seed.bits,method:meta.method,notes,warnings}
}
function hodlHexEntropy(value,targetWords=Pt){return hodlNumberBaseEntropy(value,"hex",targetWords)}
function hodlBinaryEntropy(value,targetWords=Pt){return hodlNumberBaseEntropy(value,"bin",targetWords)}
...
function hodlParseCards(raw,targetWords=Pt){
  ...
  for(let entry of entries){
    let card=hodlNormalizeCardToken(entry.token);entry.card=card;
    if(!card){invalid.push(entry.token);invalidEntries.push(entry);continue}
    let pool=cards.length<needed.first?cards:cards.slice(needed.first);
    if(pool.includes(card)){entry.duplicate=!0;duplicates.push(card);duplicateEntries.push(entry)}
...
function hodlCardTokenCanContinue(token){return/^(?:[A2-9TJQK]|1|10)$/i.test(String(token??""))}
...
function hodlCardTypedCharactersAllowed(value){return[...String(value??"")].every(character=>/[A2-9TJQKCDHS10\s,.;:_|/\-\u2660\u2663\u2665\u2666]/i.test(character))}
...
function hodlRenderCardInputState(input,targetWords=Pt){
  let analysis=hodlAnalyzeCardInput(input,targetWords),invalid=analysis.invalidRanges.length>0;input.classList.toggle("bad",invalid);input.setAttribute("aria-invalid",String(invalid));hodlRenderInputHighlight(input,analysis.invalidRanges);return analysis
}
...
function hodlUpdateCards(){
  ...
  let config=hodlSeedConfig(),parsed=hodlRenderCardInputState(input,config.words),required=parsed.needed.first+parsed.needed.extra,entropy=hodlCardsEntropy(input.value,config.words),showCards=document.getElementById("show-cards")?.checked===!0;
  ...
  try{if(parsed.cards.length&&!parsed.invalid.length&&!parsed.duplicates.length&&!parsed.pending)preview=_n(Z(new TextEncoder().encode(parsed.hashInput)).slice(0,config.bytes)).split(" ")}catch{}
  hodlRenderDiceWordGrid(wordsBox,preview,config.words,parsed.cards.length<required);
  let meta=W("#cards-meta"),missing=Math.max(0,required-parsed.cards.length),extra=Math.max(0,parsed.cards.length-required),status=!parsed.cards.length?`0 of ${required} recommended cards \u00b7 0.0 bits estimated \u00b7 Hashed card transcript`:missing?`${parsed.cards.length} of ${required} recommended cards \u00b7 ${parsed.bits.toFixed(1)} bits estimated \u00b7 seed available for testing \u00b7 ${missing} more recommended`:`${parsed.cards.length} card${parsed.cards.length===1?"":"s"} \u00b7 ${parsed.bits.toFixed(1)} bits estimated \u00b7 ready to derive${extra?` \u00b7 all ${extra} extra card${extra===1?" is":"s are"} included`:""}`;
  if(config.words===24&&parsed.cards.length>=52&&missing)status+=parsed.cards.length===52?` \u00b7 shuffle again, then deal 6 more`:` \u00b7 second shuffle ${parsed.cards.length-52} of 6`;
  if(parsed.pending)status+=` \u00b7 finish ${parsed.pending.token} with a suit`;
  if(parsed.invalidEntries.length-(parsed.pending?1:0)>0){let count=parsed.invalidEntries.length-(parsed.pending?1:0);status+=` \u00b7 ${count} invalid card${count===1?"":"s"} highlighted \u00b7 use AS, 10H, or TD`}
  if(parsed.duplicateEntries.length)status+=` \u00b7 repeated ${parsed.duplicateEntries[0].card} highlighted \u00b7 deal a different card`;
  let invalid=parsed.invalidRanges.length>0;meta.textContent=status;meta.className="muted"+(invalid?" err":!missing&&entropy.ok?" ok":"");
...
function hodlBase64KeyboardToggleMarkup(){return hodlKeyboardToggleMarkup("base64-keyboard-toggle","on-screen Base64 keyboard","base64-keyboard")}
var hodlSeedKeyboardLayouts={lower:["abcdefghij","klmnopqrs","tuvwxyz"],upper:["ABCDEFGHIJ","KLMNOPQRS","TUVWXYZ"],number:["1234567890","!@#$%^&*()","-_+=/?\\"]};
function hodlKeyboardMarkup(passphraseOnly=!1,inputName=passphraseOnly?"passphrase":"seed phrase",keyboardId="seed-keyboard",privateInitialOptions=!1){
...
function hodlPrivateKeyKeyboardMarkup(){return hodlKeyboardMarkup(!0,"private key","seed-keyboard",!0)}
function hodlBase64KeyboardMarkup(){return hodlKeyboardMarkup(!0,"Base64 entropy","base64-keyboard")}
function hodlSetOnScreenKeyboardOpen(open){
  hodlOnScreenKeyboardOpen=Boolean(open);document.querySelectorAll("[data-on-screen-keyboard-toggle]").forEach(toggle=>{toggle.setAttribute("aria-expanded",String(hodlOnScreenKeyboardOpen));let target=toggle.id==="passphrase-keyboard-toggle"?"passphrase":toggle.id==="private-keyboard-toggle"?"private key":toggle.id==="base64-keyboard-toggle"?"Base64":"seed";toggle.setAttribute("aria-label",`${hodlOnScreenKeyboardOpen?"Hide":"Show"} on-screen ${target} keyboard`)});document.querySelectorAll("[data-on-screen-keyboard]").forEach(keyboard=>{keyboard.hidden=!hodlOnScreenKeyboardOpen})
}
...
function hodlUpdateBase64KeyboardKeys(input){
  let keyboard=document.getElementById("base64-keyboard");if(!keyboard||!input)return;let analysis=hodlAnalyzeEntropyInput(input.value,"base64",Pt),definition=analysis.meta;
  keyboard.querySelectorAll("[data-seed-character-key]").forEach(button=>{let character=button.dataset.seedKey||"",remainder=definition.remainderBits&&analysis.count>=definition.fullDigits,invalid=!definition.alphabet.includes(character)||analysis.count>=definition.digits||remainder&&!definition.finalCharacters.includes(character);button.disabled=invalid});
  let space=keyboard.querySelector(".seed-keyboard-space");if(space)space.disabled=!input.value||/\s$/.test(input.value)||analysis.count>=definition.digits;let remove=keyboard.querySelector("[data-seed-delete]"),start=input.selectionStart??input.value.length,end=input.selectionEnd??start;if(remove)remove.disabled=start===end&&start===0
}
...
function hodlUpdatePrivateKeyInitialKeys(keyboard,input,kind,network){
  let row=keyboard.querySelector("[data-private-key-initial-row]");if(!row)return!1;let options=input.value.length?[]:hodlPrivateKeyInitialCharacters(kind,network),show=options.length>0,wasShowing=keyboard.classList.contains("private-key-initial-options"),modeButton=keyboard.querySelector("[data-seed-keyboard-mode]");
  if(!show&&wasShowing&&input.value&&modeButton){let first=input.value[0],layout=/^[A-Z]$/.test(first)?"upper":/^[0-9]$/.test(first)?"number":"lower";if(modeButton.dataset.seedKeyboardMode!==layout)hodlSetSeedKeyboardLayout(keyboard,modeButton,layout)}
  row.hidden=!show;keyboard.classList.toggle("private-key-initial-options",show);keyboard.querySelectorAll("[data-seed-keyboard-row],.seed-keyboard-space-row").forEach(section=>{section.hidden=show});
  row.querySelectorAll("[data-private-key-initial]").forEach((button,index)=>{let character=options[index]||"";button.hidden=!character;button.disabled=!character;if(character){button.setAttribute("data-seed-character-key","");button.dataset.seedKey=character;button.textContent=character;button.setAttribute("aria-label",`Enter ${character}`);button.removeAttribute("aria-hidden")}else{button.removeAttribute("data-seed-character-key");delete button.dataset.seedKey;button.textContent="";button.removeAttribute("aria-label");button.setAttribute("aria-hidden","true")}});
  if(show)keyboard.setAttribute("aria-label",`Choose the first ${kind==="wif"?"WIF":"Mini key"} character`);return show
}
function hodlUpdatePrivateKeyKeyboardKeys(input){
  let keyboard=document.getElementById("seed-keyboard");if(!keyboard||!input)return;let kind=hodlNormalizePrivateKeyKind(document.querySelector('input[name="kk"]:checked')?.value,input.value),network=hodlSelectedNetwork(document.getElementById("network")),initialOnly=hodlUpdatePrivateKeyInitialKeys(keyboard,input,kind,network);
  let hexKeypad=keyboard.querySelector("[data-private-key-hex-keypad]"),hexOnly=kind==="hex-key";if(hexKeypad)hexKeypad.hidden=!hexOnly;keyboard.classList.toggle("private-key-hex-options",hexOnly);if(hexOnly)keyboard.querySelectorAll("[data-seed-keyboard-row],.seed-keyboard-space-row").forEach(section=>{section.hidden=!0});else if(!initialOnly)keyboard.querySelectorAll("[data-seed-keyboard-row],.seed-keyboard-space-row").forEach(section=>{section.hidden=!1});
  keyboard.querySelectorAll("[data-seed-keyboard-row] [data-seed-character-key],[data-private-key-hex-character]").forEach(button=>{button.disabled=!hodlPrivateKeyboardCanEnterCharacter(input,button.dataset.seedKey)});let space=keyboard.querySelector(".seed-keyboard-space");if(space)space.disabled=kind!=="brain";
  let start=input.selectionStart??input.value.length,end=input.selectionEnd??start;keyboard.querySelectorAll("[data-seed-delete]").forEach(remove=>{remove.disabled=start===end&&start===0});
  if(hexOnly)keyboard.setAttribute("aria-label","On-screen hexadecimal private key keyboard");else if(!initialOnly)keyboard.setAttribute("aria-label",`On-screen ${keyboard.dataset.seedKeyboardLayout||"lower"} private key keyboard`)
}
...
function hodlBindSeedKeyboard(input,targetWords=Pt){
  ...
  keyboard.querySelectorAll("[data-seed-character-key],.seed-keyboard-space").forEach(button=>{button.onclick=()=>hodlApplySeedKeyboardKey(activeInput,button.dataset.seedKey||"")});keyboard.querySelectorAll("[data-seed-delete]").forEach(button=>hodlBindSeedKeyboardDelete(()=>activeInput,button));if(modeButton)modeButton.onclick=()=>{hodlCycleSeedKeyboardLayout(keyboard,modeButton);keyboard.setAttribute("aria-label",`On-screen ${keyboard.dataset.seedKeyboardLayout} passphrase keyboard`);refresh()};
...
function hodlBindPassphraseKeyboard(inputId="pass",toggleId="passphrase-keyboard-toggle",inputName="passphrase"){
  ...
  keyboard.querySelectorAll("[data-seed-character-key],.seed-keyboard-space").forEach(button=>{button.onclick=()=>hodlApplySeedKeyboardKey(input,button.dataset.seedKey||"")});keyboard.querySelectorAll("[data-seed-delete]").forEach(button=>hodlBindSeedKeyboardDelete(()=>input,button));if(modeButton){modeButton.disabled=!1;modeButton.onclick=()=>{hodlCycleSeedKeyboardLayout(keyboard,modeButton);keyboard.setAttribute("aria-label",`On-screen ${keyboard.dataset.seedKeyboardLayout} ${inputName} keyboard`);refresh()}};
...
function hodlBindBase64Keyboard(input){
  let toggle=document.getElementById("base64-keyboard-toggle"),keyboard=document.getElementById("base64-keyboard"),modeButton=keyboard?.querySelector("[data-seed-keyboard-mode]");if(!toggle||!keyboard||!input)return;let refresh=()=>hodlUpdateBase64KeyboardKeys(input);
  toggle.onclick=()=>{hodlSetOnScreenKeyboardOpen(!hodlOnScreenKeyboardOpen);refresh()};keyboard.querySelectorAll("button").forEach(button=>{button.addEventListener("pointerdown",event=>{event.preventDefault();input.focus({preventScroll:!0})})});keyboard.querySelectorAll("[data-seed-character-key],.seed-keyboard-space").forEach(button=>{button.onclick=()=>hodlApplySeedKeyboardKey(input,button.dataset.seedKey||"")});keyboard.querySelectorAll("[data-seed-delete]").forEach(button=>hodlBindSeedKeyboardDelete(()=>input,button));if(modeButton){modeButton.disabled=!1;modeButton.onclick=()=>{hodlCycleSeedKeyboardLayout(keyboard,modeButton);keyboard.setAttribute("aria-label",`On-screen ${keyboard.dataset.seedKeyboardLayout} Base64 entropy keyboard`);refresh()}};
  ["input","focus","click","keyup","select"].forEach(type=>input.addEventListener(type,refresh));refresh()
}
...
function hodlNumberBasePreviewWords(value,format,targetWords=Pt){
  let config=hodlSeedConfig(targetWords),analysis=hodlAnalyzeEntropyInput(value,format,config.words);if(!analysis.count||analysis.invalidCharacterCount||analysis.finalInvalid)return[];let bits=hodlNumberBaseBits(value,format,config.words).slice(0,config.bits);
  if(analysis.ready){let bytes=new Uint8Array(config.bytes);for(let index=0;index<bytes.length;index++)bytes[index]=Number.parseInt(bits.slice(index*8,index*8+8),2);try{return _n(bytes).split(" ")}catch{return[]}}
  let words=[],completeGroups=Math.min(config.partialWords,Math.floor(bits.length/11));for(let index=0;index<completeGroups;index++)words.push(Ae[Number.parseInt(bits.slice(index*11,index*11+11),2)]);return words
}
function hodlBinaryPreviewWords(value,targetWords=Pt){return hodlNumberBasePreviewWords(value,"bin",targetWords)}
function hodlHexPreviewWords(value,targetWords=Pt){return hodlNumberBasePreviewWords(value,"hex",targetWords)}
function hodlSetNumberBaseSyncStatus(synced){let status=document.getElementById("number-base-sync-status");if(status)status.hidden=!synced}
function hodlSyncNumberBases(input,format,analysis,targetWords=Pt,sourceEdit=!0){
  let state=hodlKeys[hodlActiveKey],toggle=document.getElementById("sync-number-bases");if(state){state.syncNumberBases=Boolean(toggle?.checked);state.fields[format]=input.value}if(!toggle?.checked){if(state)state.numberBasesSynced=!1;hodlSetNumberBaseSyncStatus(!1);return!1}
  if(state&&sourceEdit){state.numberBaseSyncSource=format;state.numberBasesSynced=!1}
  let source=state?.numberBaseSyncSource||format;if(format!==source){hodlSetNumberBaseSyncStatus(Boolean(state?.numberBasesSynced));return!1}
  if(!analysis.ready){if(state)state.numberBasesSynced=!1;hodlSetNumberBaseSyncStatus(!1);return!1}
  let result=hodlNumberBaseEntropy(input.value,format,targetWords);if(!result.ok){if(state)state.numberBasesSynced=!1;hodlSetNumberBaseSyncStatus(!1);return!1}
  if(state){Object.keys(hodlEntropyFormats).forEach(id=>{state.fields[id]=hodlNumberBaseValueFromBytes(result.bytes,id,targetWords)});state.numberBasesSynced=!0}hodlSetNumberBaseSyncStatus(!0);return!0
}
...
function hodlUpdateEntropyInput(input,format,targetWords=Pt,syncContext="edit"){
  let config=hodlSeedConfig(targetWords),analysis=hodlRenderEntropyInputState(input,format,config.words),definition=analysis.meta,meta=document.getElementById("entropy-meta"),words=hodlNumberBasePreviewWords(input.value,definition.id,config.words),wordsBox=document.getElementById("entropy-words"),coinPhase=Boolean(definition.binaryRemainder&&definition.remainderBits&&analysis.count>=definition.fullDigits),coinFlipsEntered=coinPhase?Math.min(definition.remainderBits,Math.max(0,analysis.count-definition.fullDigits)):0,status=coinPhase?(analysis.ready?`${definition.fullDigits} ${definition.shortLabel} characters complete · ${coinFlipsEntered} of ${definition.remainderBits} coin flips entered`:`${definition.fullDigits} ${definition.shortLabel} characters complete · coin flip ${Math.min(definition.remainderBits,coinFlipsEntered+1)} of ${definition.remainderBits} · Heads (0) or Tails (1)`):`${analysis.count} of ${analysis.limit} ${definition.unit} · ${words.length} of ${config.words} seed words filled`;
  if(analysis.invalidCharacterCount)status+=` · ${analysis.invalidCharacterCount} invalid character${analysis.invalidCharacterCount===1?"":"s"} highlighted`;
  if(analysis.finalInvalid)status+=definition.binaryRemainder?` · final ${definition.remainderBits} entropy bits must each be 0 or 1`:` · final ${definition.remainderBits}-bit character must be one of ${[...definition.finalCharacters].join(", ")}`;
  if(analysis.excessCount)status+=` · ${analysis.excessCount} extra highlighted · remove to continue`;
  if(analysis.ready)status+=" · ready to derive";
  if(meta){meta.textContent=status;meta.className="muted"+(analysis.ready?" ok":analysis.invalidRanges.length?" err":"")}
  hodlRenderDiceWordGrid(wordsBox,words,config.words,!1);
  let entropyPad=input.closest("#form")?.querySelector(".entropy-keypad");if(entropyPad)entropyPad.classList.toggle("coin-phase",coinPhase);
  input.closest("#form")?.querySelectorAll("[data-entropy-digit]").forEach(button=>{let digit=button.dataset.entropyDigit,binary=digit==="0"||digit==="1",mixedFinalPhase=Boolean(!definition.binaryRemainder&&definition.remainderBits&&analysis.count===definition.digits-1),finalRestricted=(coinPhase||mixedFinalPhase)&&!definition.finalCharacters.includes(digit);button.disabled=Boolean(finalRestricted);button.hidden=Boolean(coinPhase&&!binary);button.classList.toggle("coin-button",coinPhase&&binary);button.textContent=coinPhase&&binary?(digit==="0"?"Heads (0)":"Tails (1)"):digit;button.setAttribute("aria-label",coinPhase&&binary?(digit==="0"?"Enter Heads as binary 0":"Enter Tails as binary 1"):`Enter ${definition.shortLabel} ${digit}`);button.title=finalRestricted?(coinPhase?`The remaining ${definition.remainderBits} entropy bit${definition.remainderBits===1?"":"s"} must use 0 or 1.`:`The final character must be one of ${[...definition.finalCharacters].join(", ")}.`):""});
...
function hodlUpdateSeedLengthControl(){
  ...
  if(Ne==="hex"){let format=hodlEntropyFormatConfig(hodlEntropyFormat,config.words);help.textContent=`${config.words} words require exactly ${format.digits} ${format.unit}.${format.remainderBits?(format.binaryRemainder?` Enter ${format.fullDigits} complete ${format.shortLabel} characters followed by ${format.remainderBits} coin flip${format.remainderBits===1?"":"s"}, using Heads (0) or Tails (1).`:` The final character contributes ${format.remainderBits} bit${format.remainderBits===1?"":"s"} and must be one of ${[...format.finalCharacters].join(", ")}.`):""}`;return}
...
function hodlRenderKeyForm(){
  ...
  if(Ne==="dice"){
    let dplusFaces=["1","2","3","4","5","6","7","8","9","A","B","C","D","E","F","0"],dplusPad=dplusFaces.map(face=>{let decimal=face==="0"?16:Number.parseInt(face,16),label=hodlDPlusNumberedD16&&decimal>=10?`<span class="dplus-key-face">${decimal}</span><span class="dplus-key-decimal">${face}</span>`:hodlDPlusNumberedD16?String(decimal):face,aria=hodlDPlusNumberedD16?`D16 result ${decimal}, entered as ${face}`:`D16 result ${face}${/^[A-F]$/.test(face)?`, decimal ${decimal}`:""}`;return`<button type="button" data-d="${face}" aria-label="${aria}">${label}</button>`}).join("");
    ...
    let diceHelp=ge==="dplus"?`For each of the first ${config.partialWords} words, enter the D8 result, then both D16 results. ${config.words===24?"One final D8 roll selects the checksum word.":config.words===12?"One final D8 roll and one D16 roll select the checksum word.":"One final D16 roll and one coin flip select the checksum word: on the flip, 1–4 is Heads and 5–8 is Tails."}`:ge==="bitbox"?`${config.partialWords} lookup-table words fill one slot at a time, then choose a confirmed final checksum word. Use 1–4 for the first five rolls. On the sixth, Heads inserts an equivalent face from 1–3 and Tails inserts one from 4–6; that numeric choice does not add entropy.`:ge==="coleman"?`Every rolled 6 becomes 0 before the complete digit string is hashed with SHA-256. This Dice [1-6] method matches the method used by Keystone. Any nonempty count produces a phrase, but use at least ${config.hashRolls} fair rolls before relying on it.`:`The original dice digit string is hashed with SHA-256. This Base 10 [0-9] method matches COLDCARD and SeedSigner. Any nonempty count produces a phrase, but use at least ${config.hashRolls} fair rolls before relying on it.`;
    let dicePlaceholder=ge==="dplus"?`e.g. ${hodlDPlusNumberedD16?"1 16 16  2 10 15":"100 2AF"} …${config.words===24?" then final D8":""}`:ge==="bitbox"?"111111 222224…":"e.g. 415263415263…";
    let dplusCoinPad=config.words===18?`<div class="dice-input-pad dplus-coin-pad" role="group" aria-label="Final coin flip"><button type="button" class="coin-button" data-d="H" data-coin="heads" data-coin-low="1" data-coin-high="4" aria-label="Final coin flip: Heads, enters a D8 face from 1 to 4">Heads (1–4)</button><button type="button" class="coin-button" data-d="T" data-coin="tails" data-coin-low="5" data-coin-high="8" aria-label="Final coin flip: Tails, enters a D8 face from 5 to 8">Tails (5–8)</button></div>`:"";
    let dicePad=ge==="dplus"?`<div class="dice-input-pad dplus">${dplusPad}</div>${dplusCoinPad}`:`<div class="dice-input-pad with-coin">${[1,3,5].map(face=>`<button type="button" data-d="${face}">${face}</button>`).join("")}<button type="button" class="coin-button" data-d="H" data-coin="heads">Heads (1–3)</button>${[2,4,6].map(face=>`<button type="button" data-d="${face}">${face}</button>`).join("")}<button type="button" class="coin-button" data-d="T" data-coin="tails">Tails (4–6)</button></div>`;
    let dplusConvention=ge==="dplus"?`<p class="label" id="dplus-die-label">Which D16 dice are you rolling?</p><div class="card-suit-pad dplus-die-pad" id="dplus-die" role="group" aria-labelledby="dplus-die-label"><button type="button" class="${hodlDPlusNumberedD16?"":"active"}" data-dplus-die="hex" aria-pressed="${hodlDPlusNumberedD16?"false":"true"}">Hex D16 · 0–F</button><button type="button" class="${hodlDPlusNumberedD16?"active":""}" data-dplus-die="numbered" aria-pressed="${hodlDPlusNumberedD16?"true":"false"}">Decimal D16 · 1–16</button></div>`:"";
    at.innerHTML=`
  ...
  if(Ne==="cards"){
    ...
    input.onbeforeinput=event=>{if(event.inputType==="insertText"&&event.data&&!hodlCardTypedCharactersAllowed(event.data))event.preventDefault()};
    input.oninput=()=>{hodlApplyFilteredInput(input,hodlFilterCards);hodlUpdateCards()};input.onscroll=()=>hodlSyncDiceHighlight(input);
    ...
    document.getElementById("show-cards").onchange=event=>{let visible=event.currentTarget.checked,state=hodlKeys[hodlActiveKey],dealt=document.getElementById("dealt-cards");if(state)state.showCards=visible;if(dealt)dealt.hidden=!visible};
  ...
  if(Ne==="hex"){
    ...
    let formatChoices=["bin","base4","base8","hex","base32","base64"].map(id=>{let option=hodlEntropyFormats[id];return`<label class="choice"><input type="radio" name="entropy-format" value="${id}" ${format.id===id?"checked":""} /><span><strong>${option.label}</strong><span class="desc">${descriptions[id]}</span></span></label>`}).join("");
    ...
    at.querySelectorAll('input[name="entropy-format"]').forEach(radio=>{radio.onchange=()=>{
      ...
      hodlEntropyFormat=hodlNormalizeEntropyFormat(radio.value);if(state)state.entropyFormat=hodlEntropyFormat;
      hodlInvalidateLiveKeyResult();let error=document.getElementById("error");if(error)error.textContent="";
      hodlRenderKeyForm();hodlRestoreFormFields(state);hodlUpdateSeedLengthControl();hodlQueueMasterFingerprintPreview(0)
    }});
    let syncToggle=document.getElementById("sync-number-bases");if(syncToggle)syncToggle.onchange=()=>{if(state)state.syncNumberBases=syncToggle.checked;let input=document.getElementById(inputId);if(input)hodlUpdateEntropyInput(input,format.id);if(!syncToggle.checked)hodlSetNumberBaseSyncStatus(!1)};
    hodlBindKeyFields();let entropyInput=document.getElementById(inputId);if(entropyInput){at.querySelectorAll("[data-entropy-digit]").forEach(button=>{button.onclick=()=>hodlInsertEntropyControl(entropyInput,button)});if(format.id==="base64")hodlBindBase64Keyboard(entropyInput)}hodlRenderPassphraseKeyboard();return
  }
  if(Ne==="seed"){
...
function hodlUpdateDice(){
  ...
  if(ge==="dplus"){
    let result=inputState.dplus||hodlDPlusRolls(input.value,config.words),status="",selectingFinal=result.waiting==="last-word",d16Range=hodlDPlusNumberedD16?"1–16":"0–9 or A–F";
    ...
    if(result.waiting==="d8")status=`${result.completedGroups} of ${config.partialWords} groups entered \u00b7 word ${result.activeGroupIndex+1} \u00b7 D8 roll (1\u20138)`;
      ...
      let invalid=result.firstInvalid,position=invalid?.final?(config.words===18?(invalid.position===1?"the final coin flip":"the final D16 checksum roll"):"the final D8 checksum roll"):`word ${(invalid?.groupIndex??0)+1}'s ${invalid?.position===0?"D8":invalid?.position===1?"first D16":"second D16"} roll`;
      status=`${result.completedGroups} of ${config.partialWords} groups entered \u00b7 correct ${result.invalidRequiredCount} highlighted invalid result${result.invalidRequiredCount===1?"":"s"}, starting with ${position}`
    }
    ...
    else if(result.waiting==="checksum-d16")status=`${config.partialWords} of ${config.partialWords} word rolls complete \u00b7 final D16 checksum roll (${d16Range})`;
    else if(result.waiting==="checksum-coin")status=`${config.partialWords} of ${config.partialWords} word rolls complete \u00b7 final coin flip (1–4 Heads, 5–8 Tails)`;
...
function hodlBindKeyFields(){
  ...
  let format=hodlNormalizeEntropyFormat(hodlEntropyFormat),entropy=document.getElementById(format);if(entropy){let definition=hodlEntropyFormats[format],update=(syncContext="edit")=>{hodlApplyFilteredInput(entropy,value=>hodlFilterNumberBase(value,format));if(format==="bin")hodlFormatBinaryInput(entropy);hodlUpdateEntropyInput(entropy,format,Pt,syncContext)};entropy.setAttribute("inputmode",definition.base<=10?"numeric":"text");entropy.setAttribute("spellcheck","false");if(format==="bin")entropy.onbeforeinput=event=>hodlHandleBinarySeparatorDelete(entropy,event);entropy.oninput=()=>update(entropy.hodlRestoring?"restore":"edit");entropy.onscroll=()=>hodlSyncDiceHighlight(entropy);update("")}
  let key=document.getElementById("key");if(key){let state=hodlKeys[hodlActiveKey],values=hodlPrivateKeyValues(state?.fields||{}),selected=document.querySelector("input[name=kk]:checked"),initialKind=hodlNormalizePrivateKeyKind(selected?.value,"");key.dataset.privateKeyKind=initialKind;key.value=values[initialKind]||"";
    let apply=event=>{let selected=document.querySelector("input[name=kk]:checked"),kind=hodlNormalizePrivateKeyKind(selected?.value,key.value),pasted=event?.inputType==="insertFromPaste";if(pasted&&kind!=="brain"){let detected=hodlDetectPrivateKeyKind(key.value);if(detected&&detected!==kind){let radio=document.querySelector(`input[name="kk"][value="${detected}"]`);if(radio){radio.checked=!0;kind=detected}}}if(kind!=="brain")key.value=hodlFilterKey(key.value,kind);key.dataset.privateKeyKind=kind;values[kind]=key.value;if(state){state.fields.keyKind=kind;state.fields.key=""}hodlUpdatePrivateKeyInputPresentation();hodlRenderPrivateKeyInputState(key);hodlUpdatePrivateKeyKeyboardKeys(key)};
    let change=event=>{if(!event.currentTarget.checked)return;let previousKind=hodlNormalizePrivateKeyKind(key.dataset.privateKeyKind||"wif",key.value),nextKind=hodlNormalizePrivateKeyKind(event.currentTarget.value,"");values[previousKind]=key.value;key.dataset.privateKeyKind=nextKind;key.value=values[nextKind]||"";apply();key.setSelectionRange(key.value.length,key.value.length)};
    key.oninput=apply;key.onscroll=()=>hodlSyncDiceHighlight(key);document.querySelectorAll("input[name=kk]").forEach(radio=>{radio.addEventListener("input",change);radio.addEventListener("change",change)});document.getElementById("network")?.addEventListener("change",apply);apply()}
}
...
function hodlCanDeriveCurrentKey(){
  try{
    ...
    if(Ne==="dice"){
      ...
      if(ge==="dplus"){
        let rollsFinalWord=!0,parsed=analysis.dplus||hodlDPlusRolls(input.value,Pt),finalWord=rollsFinalWord?parsed.finalWord:ft;
        if(rollsFinalWord){if(!parsed.complete)return!1}
...
function hodlFingerprintMnemonic(){
  try{
    if(Ne==="dice"){
      ...
      if(ge==="dplus"){
        let rollsFinalWord=!0,parsed=hodlDPlusRolls(input.value,Pt),finalWord=rollsFinalWord?parsed.finalWord:ft;
...
function hodlInitMasterFingerprintPreview(){
  ...
  panel.addEventListener("input",event=>{
    let id=event.target?.id;if(!["pass","dice","hex","bin","base4","base8","base32","base64","seed","cards"].includes(id))return;
  ...
  panel.addEventListener("change",event=>{
    let target=event.target;if(!(target instanceof Element)||!target.matches('input[name="dm"], input[name="entropy-format"], select[aria-label^="Valid final word"]'))return;
    hodlInvalidateLiveKeyResult();hodlQueueMasterFingerprintPreview()
  });
  panel.addEventListener("click",event=>{
    let target=event.target instanceof Element?event.target.closest("#modes button, [data-seed-words], [data-dplus-die], [data-d], [data-lw], [data-card-suit], [data-card-rank], #card-undo"):null;if(!target)return;
    hodlInvalidateLiveKeyResult();hodlQueueMasterFingerprintPreview()
  });
  ...
  try{
    ...
    if(Ne==="dice"){
      if(ge==="dplus"){
        ...
        if(parsed.firstInvalid){let invalid=parsed.firstInvalid,position=invalid.final?(Pt===18?(invalid.position===1?"the final coin flip":"the final D16 checksum roll"):"the final D8 checksum roll"):`word ${invalid.groupIndex+1}'s ${invalid.position===0?"D8":invalid.position===1?"first D16":"second D16"} roll`;throw new Error(`Correct the highlighted invalid result in ${position}. Each D++ word keeps its original three-character group.`)}
        ...
        let rollsFinalWord=!0,finalWord=rollsFinalWord?parsed.finalWord:ft;if(!rollsFinalWord&&(!finalWord||!parsed.candidates.includes(finalWord)))throw new Error(`Choose one of the ${hodlSeedConfig().candidates} valid final checksum words before deriving the wallet.`);
        if(rollsFinalWord&&!parsed.complete)throw new Error("Complete all D++ rolls before deriving the wallet.");
        let phrase=[...parsed.wordSlots,finalWord].join(" "),validation=hodlValidateTargetMnemonic(phrase,Pt);if(!validation.ok)throw new Error(validation.error);let notes=parsed.notes.slice();if(!rollsFinalWord)notes.push(`Selected checksum-valid final word: ${finalWord}.`);re=ar(phrase,passphrase,network,count,{notes,warnings:parsed.warnings},account)
      }else if(ge==="bitbox"){
...
function hodlPrivateKeyValues(fields){
  ...
  hodlPrivateKeyKinds.forEach(kind=>{if(typeof fields.privateKeys[kind]!=="string")fields.privateKeys[kind]=""});
  let legacy=String(fields.key??"");if(legacy){let kind=hodlNormalizePrivateKeyKind(fields.keyKind,legacy);if(!fields.privateKeys[kind])fields.privateKeys[kind]=legacy;fields.key=""}
  return fields.privateKeys
}
function hodlNewKeyState(name,keyId,keyNumber){let id=keyId??hodlNextKeyId++,number=keyNumber??hodlNextKeyNumber++;return{id,number,color:hodlKeyColor(id),name:name||hodlDefaultKeyName(number),mode:"dice",diceMethod:"coldcard",entropyFormat:"bin",syncNumberBases:!1,numberBaseSyncSource:"",numberBasesSynced:!1,seedAutocomplete:!1,dplusNumberedD16:!1,showCards:!1,targetWords:24,diceCoinPositions:[],lastWord:"",dplusLastWord:"",result:null,reveal:!1,accountId:"bip84",error:"",fields:{pass:"",script:"bip84",network:"mainnet",account:"0",count:"5",dice:"",dplusDice:"",hex:"",bin:"",base4:"",base8:"",base32:"",base64:"",cards:"",seed:"",key:"",keyKind:"wif",privateKeys:{wif:"","hex-key":"",minikey:"",brain:""}}}}
function hodlRestoreFormFields(state){
  if(!state)return;
  let privateKeys=hodlPrivateKeyValues(state.fields),restoredKeyKind=hodlNormalizePrivateKeyKind(state.fields.keyKind,privateKeys[state.fields.keyKind]||"");state.fields.keyKind=restoredKeyKind;document.querySelectorAll("input[name=kk]").forEach(input=>{input.checked=input.value===restoredKeyKind});
  let syncNumberBases=document.getElementById("sync-number-bases");if(syncNumberBases)syncNumberBases.checked=Boolean(state.syncNumberBases);
  ...
  hodlDPlusNumberedD16=Boolean(state.dplusNumberedD16);hodlUpdateDPlusDieControl();
  ["dice","hex","bin","base4","base8","base32","base64","seed","key","cards"].forEach(id=>{let el=document.getElementById(id);if(el){el.value=id==="dice"&&ge==="dplus"?state.fields.dplusDice||"":id==="key"?privateKeys[restoredKeyKind]||"":state.fields[id]||"";if(id==="key")el.dataset.privateKeyKind=restoredKeyKind;if(id==="dice"){el.dataset.previousValue=el.value;el.setSelectionRange(el.value.length,el.value.length)}el.hodlRestoring=!0;el.dispatchEvent(new Event("input"));delete el.hodlRestoring}});
}
function hodlSetMode(mode){
  hodlCaptureKey();
  let state=hodlKeys[hodlActiveKey];if(state)state.mode=mode;
  Ne=mode;hodlEntropyFormat=hodlNormalizeEntropyFormat(state?.entropyFormat);[...Zs.children].forEach((button,index)=>{let active=hodlKeyModes[index]===Ne;button.classList.toggle("active",active);button.setAttribute("aria-pressed",String(active))});
...
function hodlKeyStateNeedsClear(state){
  if(!state)return!1;let fields=state.fields||{},privateKeys=hodlPrivateKeyValues(fields),hasText=id=>String(fields[id]??"").length>0;
  return String(state.mode??"dice")!=="dice"||String(state.diceMethod??"coldcard")!=="coldcard"||String(state.entropyFormat??"bin")!=="bin"||Boolean(state.syncNumberBases)||Boolean(state.seedAutocomplete)||Boolean(state.dplusNumberedD16)||Boolean(state.showCards)||Number(state.targetWords??24)!==24||
    ...
    hodlNormalizePrivateKeyKind(fields.keyKind,privateKeys[fields.keyKind]||"")!=="wif"||["pass","dice","dplusDice","hex","bin","base4","base8","base32","base64","cards","seed","key"].some(hasText)||hodlPrivateKeyKinds.some(kind=>privateKeys[kind].length>0)
}
...
function hodlCaptureKey(){
  ...
  state.mode=Ne;state.diceMethod=ge;state.entropyFormat=hodlEntropyFormat;let syncNumberBases=document.getElementById("sync-number-bases");if(syncNumberBases)state.syncNumberBases=syncNumberBases.checked;let seedAutocomplete=document.getElementById("seed-autocomplete");if(seedAutocomplete)state.seedAutocomplete=seedAutocomplete.checked;state.dplusNumberedD16=Boolean(hodlDPlusNumberedD16);let showCards=document.getElementById("show-cards");if(showCards)state.showCards=showCards.checked;state.targetWords=Pt;state.diceCoinPositions=hodlDiceCoinPositions.slice();if(ge==="dplus")state.dplusLastWord=ft;else if(ge==="bitbox")state.lastWord=ft;state.result=re;state.reveal=Ge;state.accountId=hodlSelectedScriptType();state.fields.script=state.accountId;
  state.error=document.getElementById("error")?.textContent||"";
  ["pass","account","count","hex","bin","base4","base8","base32","base64","seed","cards"].forEach(id=>{let el=document.getElementById(id);if(el)state.fields[id]=el.value});state.fields.network=hodlSelectedNetwork(document.getElementById("network"));
  let dice=document.getElementById("dice");if(dice)state.fields[ge==="dplus"?"dplusDice":"dice"]=dice.value;
  let key=document.getElementById("key"),privateKeys=hodlPrivateKeyValues(state.fields),checkedKeyKind=document.querySelector("input[name=kk]:checked")?.value||state.fields.keyKind,keyKind=hodlNormalizePrivateKeyKind(key?.dataset.privateKeyKind||checkedKeyKind,key?.value||"");if(key)privateKeys[keyKind]=key.value;state.fields.keyKind=keyKind;state.fields.key="";
}
...
function hodlRestoreKey(){
  let state=hodlKeys[hodlActiveKey];
  if(!state){
    Ne="dice";ge="coldcard";hodlEntropyFormat="bin";hodlDPlusNumberedD16=!1;Pt=24;hodlDiceCoinPositions=[];ft="";re=null;Ge=!1;hodlAccountId="bip84";
  ...
  Ne=state.mode;ge=state.diceMethod;hodlEntropyFormat=hodlNormalizeEntropyFormat(state.entropyFormat);hodlDPlusNumberedD16=Boolean(state.dplusNumberedD16);Pt=hodlSeedLengths[Number(state.targetWords)]?Number(state.targetWords):24;hodlDiceCoinPositions=hodlNormalizeDiceCoinPositions(state.diceCoinPositions);ft=ge==="dplus"?state.dplusLastWord||"":ge==="bitbox"?state.lastWord||"":"";
...
function hodlInitSecretFieldAutoClear(){
  let clearSecretFields=()=>{
    for(let id of["dice","hex","bin","base4","base8","base32","base64","seed","key","pass","cards"]){let field=document.getElementById(id);if(field)field.value=""}
