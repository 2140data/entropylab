    ...
    <section class="card no-print" id="msig-card" role="tabpanel" hidden>
      ...
      <fieldset class="msig-threshold-control">
        ...
        <div class="msig-threshold-labels">
          ...
          <label for="msig-n-number"><span>Total signing keys (n)</span><input class="msig-threshold-number" id="msig-n-number" type="number" min="2" max="15" step="1" value="3" inputmode="numeric" aria-describedby="msig-threshold-help"></label>
        </div>
        <div class="msig-threshold-slider" id="msig-threshold-slider" style="--msig-m-position:12.5%;--msig-n-position:25%" data-slider-max="9">
          ...
          <input class="msig-threshold-range" id="msig-n" type="range" min="2" max="15" step="1" value="3" aria-label="Total signing keys (n)" aria-describedby="msig-threshold-help">
        </div>
...
function hodlMultisigAccountNumber(origin,kind){
  if(kind==="p2sh")return null;
  let steps=hodlNormalizeOriginPath(origin?.path).split("/"),match=steps[2]?.match(/^(\d+)h$/);
  if(!match)throw new Error("BIP48 account index must be hardened.");
  let account=Number(match[1]);
  if(!Number.isSafeInteger(account)||account<0||account>0x7fffffff)throw new Error("BIP48 account index is out of range.");
  return account
}
...
function hodlDetectMsigScriptSummary(values=hodlReadMsigXpubs()){
  ...
  for(let raw of values){
    ...
    try{let evidence=hodlMultisigScriptEvidence(hodlParseMultisigCosigner(raw));if(evidence.prefixKind)kinds.push(evidence.prefixKind);if(evidence.originKind)kinds.push(evidence.originKind)}catch{}
  }
...
function hodlMultisigKeyPlaceholder(kind,network){
  let testnet=network==="testnet",coin=testnet?"1h":"0h";
  if(kind==="p2sh")return`[fingerprint/45h/0]${testnet?"tpub":"xpub"}…`;
...
function hodlUpdateMsigKeyPlaceholders(){
  let kind=hodlScriptKind(),network=hodlSelectedNetwork(document.getElementById("msig-network")),placeholder=hodlMultisigKeyPlaceholder(kind,network);
...
function hodlUpdateMsigScriptDetection(){
  ...
  let summary=hodlDetectMsigScriptSummary(),desired=summary.kind;
  if(desired==="mixed"){
...
function hodlUpdateMsigAccount(){
  ...
  let kind=hodlScriptKind(),help=document.getElementById("msig-account-help"),warning=document.getElementById("msig-account-warning");
  if(kind==="p2sh"){
    field.value="";field.placeholder="Not applicable";field.dataset.state="not-applicable";
    if(help)help.textContent="BIP45 co-signer branches do not contain an account number.";
  ...
  if(help)help.textContent=summary.mixed?"Co-signer key origins use different account numbers.":summary.account==null?"Derived from co-signer key origins.":"Derived from the co-signer account paths.";
  if(warning){warning.textContent=message;warning.hidden=!message}
  return summary
}
...
function hodlRenderMsigThreshold(){
  ...
  if(mNumber){mNumber.value=String(m);mNumber.min="1";mNumber.max=String(hodlMsigSliderLimit)}if(nNumber){nNumber.value=String(n);nNumber.min="2";nNumber.max=String(hodlMsigSliderLimit)}
  mInput.setAttribute("aria-valuetext",m+" signature"+(m===1?"":"s")+" needed");nInput.setAttribute("aria-valuetext",n+" total signing keys");
...
function hodlSetMsigThresholds(mValue,nValue,changed,moveOther){
  ...
  let n=hodlClampMsigThreshold(nValue,2,hodlMsigSliderLimit),m=hodlClampMsigThreshold(mValue,1,hodlMsigSliderLimit);
...
function hodlBindMsigThresholdSlider(){
  ...
  let bindNumber=(input,handle)=>{if(!input)return;let apply=commit=>{let raw=input.value.trim();if(!raw){if(commit)hodlRenderMsigThreshold();return}if(!commit&&handle==="n"&&raw==="1"&&hodlMsigSliderLimit>=10)return;hodlChangeMsigThreshold(handle,raw,!0)};input.addEventListener("input",()=>apply(!1));input.addEventListener("change",()=>apply(!0));input.addEventListener("blur",()=>apply(!0));input.addEventListener("focus",()=>input.select());input.addEventListener("keydown",event=>{if(["e","E","+","-","."].includes(event.key))event.preventDefault();if(event.key==="Enter"){event.preventDefault();apply(!0);input.select()}})};
...
function hodlCheckXpub(ta){let value=ta.value.trim();if(!value){hodlHint(ta,!0,"");return}try{let parsed=hodlParseMultisigCosigner(value),network=hodlSelectedNetwork(document.getElementById("msig-network")),kind=hodlScriptKind();if(kind==="mixed")throw new Error("This key indicates a different script type from another co-signer. All co-signers must use one multisig script type.");if(parsed.isPrivate)throw new Error("Paste an extended public key, never an extended private key.");if(parsed.network!==network)throw new Error(`${parsed.prefix} is for ${parsed.network}; the multisig is set to ${network}.`);if(!hodlMultisigPrefixCompatible(parsed,kind))throw new Error(parsed.scope==="singlesig"?"Use a generic xpub/tpub here, or a proper uppercase multisig SLIP-132 export.":`${parsed.prefix} does not match the selected multisig script type.`);let accountError=hodlMultisigAccountKeyError(parsed,kind);if(accountError)throw new Error(accountError);if(!parsed.origin)throw new Error("Paste [fingerprint/48h/0h/0h/2h]xpub… so a signer can recognize this key.");let originError=hodlOriginMatchesParsedKey(parsed.origin,parsed);if(originError)throw new Error(originError);let scriptOriginError=hodlOriginScriptError(parsed.origin,kind,network);if(scriptOriginError)throw new Error(scriptOriginError);if(hodlDuplicateMultisigKey(ta,parsed))throw new Error("This duplicates another co-signer. Every co-signer must use a distinct extended public key.");hodlHint(ta,!0,`${parsed.prefix} origin, checksum, and account path look valid`)}catch(error){hodlHint(ta,!1,error.message||"Not a valid multisig extended public key")}}
function hodlResetMsigForm(){hodlSetMsigThresholds(2,3);hodlSyncSelect(document.getElementById("msig-script-type"),"p2wsh");hodlSyncSelect(document.getElementById("msig-network"),"mainnet");hodlSyncSelect(document.getElementById("msig-count"),"5");hodlFillKeys([]);document.getElementById("msig-error").textContent=""}
function hodlInitMsig(){hodlBindMsigThresholdSlider();let recheck=()=>{hodlUpdateMsigScriptDetection();hodlInvalidateMsig();document.querySelectorAll("#msig-keys textarea").forEach(hodlCheckXpub)},script=document.getElementById("msig-script-type");script.addEventListener("change",()=>{if(script.value!=="mixed")script.dataset.lastConcrete=script.value;recheck()});document.getElementById("msig-network").addEventListener("change",recheck);document.getElementById("msig-count").addEventListener("change",hodlInvalidateMsig);hodlResetMsigForm();W("#msig-go").onclick=hodlBuildMsig;W("#msig-wipe").onclick=hodlWipeActiveMsig}
...
function hodlValidatedMsigInputs(){
  ...
  let kind=hodlScriptKind(),nodes=[],xpubs=[],keyTokens=[],legacyChildNumbers=[],accountNumbers=[];
  ...
  return{network,count,n,m,kind,nodes,xpubs,keyTokens,accountSummary,accountWarning}
}
function hodlBuildMsig(){
  let error=document.getElementById("msig-error");error.textContent="";
  try{
    let{network,count,n,m,kind,nodes,xpubs,keyTokens,accountSummary,accountWarning}=hodlValidatedMsigInputs();
    ...
    re={kind:"msig",network,m,n,script:kind,account:accountSummary.account,accountMixed:accountSummary.mixed,xpubs,receiveDescriptor:Le(descriptor),changeDescriptor:Le(changeDescriptor),walletDescriptor:hodlWatchOnlyMultipathDescriptor(Le(descriptor)),receive,change,notes:["This is watch-only. Private keys never entered this calculator.","Each key origin lets a signer match its seed to one co-signer.","A signer is only needed when you spend."],warnings:accountWarning?[accountWarning]:[]};
...
function hodlShowMsig(){if(!re||re.kind!=="msig")return;Ge=!1;let accountLabel=re.accountMixed?" · Account Mixed":re.account==null?"":` · Account ${re.account}`;dr.innerHTML=`
    <section class="card account-result-card">
      <div class="kicker">${re.m}-of-${re.n} multisig · ${re.network}${accountLabel}</div>
...
function hodlRestoreMsig(){
  ...
  hodlSyncSelect(document.getElementById("msig-script-type"),state.fields.script||"p2wsh");state.fields.network=state.fields.network==="testnet"?"testnet":"mainnet";hodlSyncSelect(document.getElementById("msig-network"),state.fields.network);hodlSyncSelect(document.getElementById("msig-count"),state.fields.count||"5");hodlFillKeys(state.fields.xpubs||[]);
