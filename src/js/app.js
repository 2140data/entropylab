...
function hodlKeyboardMarkup(passphraseOnly=!1,inputName=passphraseOnly?"passphrase":"seed phrase",keyboardId="seed-keyboard"){
...
function hodlRestoreFormFields(state){
  ...
  ["dice","hex","bin","base4","base8","base32","base64","seed","key","cards"].forEach(id=>{let el=document.getElementById(id);if(el){el.value=id==="dice"&&ge==="dplus"?state.fields.dplusDice||"":state.fields[id]||"";if(id==="dice"){el.dataset.previousValue=el.value;el.setSelectionRange(el.value.length,el.value.length)}el.hodlRestoring=!0;el.dispatchEvent(new Event("input"));delete el.hodlRestoring}});
}
...
function hodlKeyStateNeedsClear(state){
  ...
  return String(state.mode??"dice")!=="dice"||String(state.diceMethod??"coldcard")!=="coldcard"||String(state.entropyFormat??"bin")!=="bin"||Boolean(state.syncNumberBases)||Boolean(state.seedAutocomplete)||Boolean(state.dplusNumberedD16)||Boolean(state.showCards)||Number(state.targetWords??24)!==24||
    ...
    hodlNormalizePrivateKeyKind(fields.keyKind,fields.key)!=="wif"||["pass","dice","dplusDice","hex","bin","base4","base8","base32","base64","cards","seed","key"].some(hasText)
}
...
function hodlCaptureKey(){
  ...
  ["pass","account","count","hex","bin","base4","base8","base32","base64","seed","key","cards"].forEach(id=>{let el=document.getElementById(id);if(el)state.fields[id]=el.value});state.fields.network=hodlSelectedNetwork(document.getElementById("network"));
